...
static cid_err_t cid_read_version_varint(const uint8_t* const bytes, size_t bytes_len, uint64_t* const version_varint, size_t* bytes_read) {
  ...
  varint_err_t varint_err = varint_to_uint64(bytes, bytes_len, version_varint, bytes_read);
  if (varint_err) {
...
cid_err_t cid_read_version(const uint8_t* const cid, size_t cid_len, uint64_t* const version) {
  return cid_read_version_varint(cid, cid_len, version, NULL);
}

static cid_err_t cid_read_content_type_varint(const uint8_t* const bytes, size_t bytes_len, uint64_t version, uint64_t* content_type,
                                              size_t* bytes_read) {
  if (version == 0) {
  ...
  if (version == 1) {
    ...
    varint_err_t varint_err = varint_to_uint64(bytes, bytes_len, &multicodec_varint, &multicodec_varint_len);
    if (varint_err) {
      return CID_ERR_INVALID_INPUT;
    }
    if (content_type != NULL) {
...
cid_err_t cid_read_content_type(const uint8_t* const cid, size_t cid_len, uint64_t* content_type) {
  size_t bytes_read = 0;
  uint64_t version = 0;
  cid_err_t err = cid_read_version_varint(cid, cid_len, &version, &bytes_read);
  if (err) {
    return err;
  }
...
cid_err_t cid_str_to_bytes(const char* const cid, uint8_t* const buf, size_t buf_len, size_t* const bytes_len) {
  unsigned long num_bytes = strlen(cid);
  uint8_t* str_bytes = (uint8_t*)cid;
  size_t str_len = utf8_len(str_bytes, num_bytes);
  if (str_len == 46 && cid[0] == 'Q' && cid[1] == 'm') {
  ...
  size_t dec_bytes = 0;
  mb_err_t dec_err = mb_decode(str_bytes, num_bytes, NULL, buf, buf_len, &dec_bytes);
  if (dec_err) {
    return CID_ERR_INVALID_INPUT;
  }
...
int main(int argc, char* argv[]) {
  ...
  size_t buf_len = 0;
  cid_err_t err = cid_str_to_bytes_len(cid_str, &buf_len);
  if (err) {
    printf("%s\n", CID_ERR_STRS[err]);
    exit_code = 1;
    goto exit;
  ...
  mh_err_t mh_err = mh_read_digest(multihash, multihash_len, &digest_size, &digest);
  if (mh_err) {
  ...
  mb_err_t mb_err = mb_encode(digest, digest_size, MB_ENC_BASE16UPPER, digest_enc_buf, digest_enc_buf_len, &digest_enc_buf_bytes);
  if (mb_err) {
