...
mb_err mb_base10_encode(const uint8_t* const input, size_t input_size, uint8_t* const result_buf, size_t result_buf_size,
  ...
  if (result_size && !result_buf) {
    // this is approximate, sometimes it can overshoot by 1 byte
    *result_size = ((input_size - num_zeros) * 242 / 100 + 1) + num_zeros;
    return MB_ERR_OK;
  }
  ...
  if (result_buf) {
    ...
    if (result_size) {
      // if we computed the result, we know the exact size
...
mb_err mb_base10_decode(const uint8_t* const input, size_t input_size, uint8_t* const result_buf, size_t result_buf_size,
  ...
  if (result_size && !result_buf) {
    *result_size = ((input_size - num_zeros) * 416 / 1000 + 1) + num_zeros;
    return MB_ERR_OK;
  }

  if (result_buf && input_size > 0) {
    size_t high = result_buf_size;
    uint32_t carry = 0;
    size_t j = 0;
    for (size_t i = num_zeros; i < input_size; i++, high = j) {
...
mb_err mb_base58btc_encode(const uint8_t* const input, size_t input_size, uint8_t* const result_buf, size_t result_buf_size,
  ...
  if (result_size && !result_buf) {
    // this is approximate, sometimes it can overshoot by 1 byte
    *result_size = ((input_size - num_zeros) * 138 / 100 + 1) + num_zeros;
    return MB_ERR_OK;
  }
...
mb_err mb_base58btc_decode(const uint8_t* const input, size_t input_size, uint8_t* const result_buf, size_t result_buf_size,
  ...
  if (result_size && !result_buf) {
    // this is approximate, sometimes it can overshoot by 1 byte
    *result_size = ((input_size - num_zeros) * 733 / 1000 + 1) + num_zeros;
    return MB_ERR_OK;
  }
...
static const mb_encoding codes[MB_NUM_ENCODINGS] = {
    {
        .code = '\0',
        .name = "identity",
        .enc = MB_ENC_IDENTITY,
...
mb_err mb_encode(const uint8_t* const input, size_t input_size, mb_enc encoding, uint8_t* result_buf, size_t result_buf_size,
                 size_t* result_size) {
  if (encoding > MB_NUM_ENCODINGS - 1) {
  ...
  mb_err err = enc.encode(input, input_size, result_buf, result_buf_size, result_size);
...
mb_err mb_encode_as(const uint8_t* input, size_t input_size, mb_enc encoding, uint8_t* result_buf, size_t result_buf_size,
                    size_t* result_size) {
  for (size_t i = 0; i < MB_NUM_ENCODINGS; i++) {
